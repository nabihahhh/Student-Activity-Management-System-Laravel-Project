    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.feedback.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('proposal') ? 'is-invalid' : '' }}" name="proposal_id" id="proposal_id" required>
                    @foreach($proposals as $id => $proposal)
                        <option value="{{ $id }}" {{ old('proposal_id') == $id ? 'selected' : '' }}>{{ $proposal }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('feedback_text') ? 'is-invalid' : '' }}" name="feedback_text" id="feedback_text" required>{{ old('feedback_text') }}</textarea>
