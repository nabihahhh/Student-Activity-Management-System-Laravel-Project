...
<div class="card">
            <div class="card-header">
                <div class="row">
                    <div class="col-md-10">
                    @section('header', 'PROGRAMME REPORT FORM (PPF) - Give Approval and Feedback ')<br>
                    </div>
                    <div class="col-md-2">
...
<div class="p-10 shadow-md rounded-md text-left bg-gray-50 w-screen mt-4">
    ...
    <span class="px-6 py-1 flex items-center leading-normal bg-green-700 text-white shadow-md rounded-md border-r-0 border-grey-light px-3 whitespace-no-wrap text-grey-dark text-sm">
                            ...
                            <strong>Programme ID:</strong> {{$report->id}}<br>
                            <strong>Programme Name: </strong>{{$report->programmeName}}<br>
                            <strong>Organiser:</strong> {{$report->programmeOrganizer}}<br>
                            <strong>Date:</strong> {{$report->date}}<br>
<br>
        <div class="card-body">
            <form method="POST" action="{{ route("admin.report.analyze", $report) }}" enctype="multipart/form-data">
                @csrf
